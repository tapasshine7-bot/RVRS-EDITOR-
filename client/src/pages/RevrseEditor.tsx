...
import { getRecordingCapabilities, nextExportPreparationProgress } from "@/lib/export-preparation";
import {
  addKeyframe,
  addMarker,
  addTrack,
  ...
  getVoiceOverStartErrorMessage,
  getVoiceOverUnavailableMessage,
  isWorkspacePanelActive,
  needsAudioRightsConfirmation,
  normalizeAssets,
  normalizeProject,
  preferredVoiceOverMimeType,
  projectDuration,
  removeClip,
  serializableProject,
  shouldKeepVoiceOverRecording,
  splitClip,
  TimelineClip,
  TimelineTrack,
  updateClip,
...
type SharedSoundResource = { id: number; title: string; description: string; category: string; moods: string; licenseType: string; creditLine: string; sourceUrl: string; url: string; originalName: string; mimeType: string; byteSize: number; durationMs: number; creatorName: string | null };
type SoundPublicationInput = { title: string; description: string; category: string; moods?: string; licenseType?: "creator-owned" | "public-domain" | "royalty-free" | "permission"; creditLine?: string; sourceUrl?: string };
...
type MusicProviderStatus = { provider: string; configured: boolean; mode: "metadata-only" };
type MusicProviderTrack = { id: string; title: string; artist: string; durationSeconds: number; artworkUrl: string; sourceUrl: string; licenseUrl: string };
...
export default function RevrseEditor() {
  ...
  const [reportResource, setReportResource] = useState<CommunityResource | null>(null);
  const [showModeration, setShowModeration] = useState(false);
  const [pendingLocalImport, setPendingLocalImport] = useState<File[] | null>(null);
  ...
  const discardVoiceOverRef = useRef(false);
  ...
  const createResourceReport = trpc.templateStudio.createReport.useMutation();
  const moderationQueue = trpc.templateStudio.moderationQueue.useQuery(undefined, { enabled: currentUser?.role === "admin", retry: false });
  const resolveResourceReport = trpc.templateStudio.resolveReport.useMutation();
  ...
  useEffect(() => {
    const openReport = (event: Event) => { const resource = (event as CustomEvent<CommunityResource>).detail; if (resource) setReportResource(resource); };
    const openModeration = () => setShowModeration(true);
    window.addEventListener("revrse:report-resource", openReport);
    window.addEventListener("revrse:open-moderation", openModeration);
    return () => { window.removeEventListener("revrse:report-resource", openReport); window.removeEventListener("revrse:open-moderation", openModeration); };
  }, []);
  ...
  function importFiles(files: FileList | File[] | null, audioRightsConfirmed = false) {
    if (!files?.length) return;
    const selectedFiles = Array.from(files);
    if (needsAudioRightsConfirmation(selectedFiles, audioRightsConfirmed)) {
      setPendingLocalImport(selectedFiles);
      return;
    }
  ...
  function confirmLocalAudioImport() {
    const files = pendingLocalImport;
    setPendingLocalImport(null);
    if (files?.length) importFiles(files, true);
  }

  function releaseVoiceOverInput() {
  ...
  async function startVoiceOver() {
    const unavailableMessage = getVoiceOverUnavailableMessage({ hasGetUserMedia: Boolean(navigator.mediaDevices?.getUserMedia), hasMediaRecorder: typeof MediaRecorder !== "undefined" });
    ...
    try {
      ...
      const mimeType = preferredVoiceOverMimeType(type => MediaRecorder.isTypeSupported(type));
      const recorder = mimeType ? new MediaRecorder(stream, { mimeType }) : new MediaRecorder(stream);
      voiceChunksRef.current = [];
      discardVoiceOverRef.current = false;
      ...
      recorder.addEventListener("stop", () => {
        ...
        const shouldDiscard = discardVoiceOverRef.current;
        const recordedType = recorder.mimeType || mimeType || "audio/webm";
        releaseVoiceOverInput();
        if (!shouldKeepVoiceOverRecording({ discard: shouldDiscard, chunkCount: chunks.length })) return;
        const blob = new Blob(chunks, { type: recordedType });
        const file = new File([blob], `voice-over-${new Date().toISOString().replace(/[:.]/g, "-")}.webm`, { type: recordedType });
        importFiles([file], true);
        toast.success("Voice-over added to this browser-local project.");
      });
      recorder.start();
      setVoiceRecording(true);
      toast.message("Recording locally. Stop when your voice-over is complete.");
    } catch (error) {
      toast.error(getVoiceOverStartErrorMessage(error instanceof DOMException ? error.name : undefined));
      releaseVoiceOverInput();
    }
  }

  function stopVoiceOver(discard = false) {
    discardVoiceOverRef.current = discard;
  ...
  async function publishLicensedSound(file: File, input: SoundPublicationInput) {
    ...
    try {
      ...
      publishSharedSound.mutate({ ...input, moods: input.moods ?? "", licenseType: input.licenseType ?? "creator-owned", creditLine: input.creditLine ?? "", sourceUrl: input.sourceUrl ?? "", originalName: file.name, mimeType: file.type as "audio/mpeg" | "audio/mp4" | "audio/wav" | "audio/ogg" | "audio/webm", base64, byteSize: file.size, ...metadata, rightsAttested: true }, {
    ...
    <div className="min-h-screen bg-[#080a0f] text-slate-100 selection:bg-cyan-300/30">
      {mode === "dashboard" ? (
        ...
        <EditorWorkspace
          ...
          onBack={() => setMode("dashboard")} onProjectName={updateProjectName} onImport={() => fileInputRef.current?.click()} voiceRecording={voiceRecording} onStartVoiceOver={startVoiceOver} onStopVoiceOver={stopVoiceOver} onExport={() => setShowExport(true)} onSettings={() => setShowSettings(true)} onShortcuts={() => setShowShortcuts(true)} onCommand={() => setShowCommand(true)} onAccountSave={saveProjectToAccount} syncing={saveServerProject.isPending}
      ...
      <input ref={fileInputRef} onChange={handleImport} type="file" multiple accept="video/*,image/*,audio/*,.srt,application/x-subrip" className="hidden" />
      ...
      {reportResource && <ReportResourceDialog resource={reportResource} accountReady={Boolean(currentUser)} pending={createResourceReport.isPending} onClose={() => setReportResource(null)} onSubmit={(input) => {
        if (!currentUser) { toast.message("Sign in to report a shared resource."); return; }
        createResourceReport.mutate({ resourceType: reportResource.type, resourceId: reportResource.id, ...input }, { onSuccess: () => { toast.success("Report sent to the moderation queue."); setReportResource(null); }, onError: error => toast.error(error.message || "Your report could not be sent.") });
      }} />}
      {currentUser?.role === "admin" && <button onClick={() => setShowModeration(true)} className="fixed bottom-4 right-4 z-30 rounded-full border border-amber-300/25 bg-[#17140c] px-3 py-2 text-xs font-semibold text-amber-100 shadow-xl transition hover:bg-[#211c0d]">Moderation queue{moderationQueue.data?.length ? ` · ${moderationQueue.data.length}` : ""}</button>}
      {showModeration && currentUser?.role === "admin" && <ModerationDialog reports={moderationQueue.data ?? []} pending={resolveResourceReport.isPending} onClose={() => setShowModeration(false)} onResolve={(id, status, moderatorNote) => resolveResourceReport.mutate({ id, status, moderatorNote }, { onSuccess: () => { toast.success("Report updated."); void utils.templateStudio.moderationQueue.invalidate(); }, onError: error => toast.error(error.message || "Report could not be updated.") })} />}
      {pendingLocalImport && <LocalAudioRightsDialog fileCount={pendingLocalImport.length} onCancel={() => setPendingLocalImport(null)} onConfirm={confirmLocalAudioImport} />}
...
function LocalAudioRightsDialog({ fileCount, onCancel, onConfirm }: { fileCount: number; onCancel: () => void; onConfirm: () => void }) {
  return <div className="fixed inset-0 z-[80] grid place-items-center bg-slate-950/80 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="local-audio-rights-title"><div className="w-full max-w-md rounded-2xl border border-cyan-300/20 bg-[#11161f] p-5 shadow-2xl"><div className="flex items-start gap-3"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-cyan-300/10 text-cyan-100"><Music2 className="h-5 w-5" /></div><div><h2 id="local-audio-rights-title" className="text-base font-semibold text-slate-100">Use original or licensed audio</h2><p className="mt-1 text-xs leading-relaxed text-slate-400">You selected {fileCount} local audio file{fileCount === 1 ? "" : "s"}. Confirm that you created the audio, own the rights, or have permission to use it in this project.</p></div></div><div className="mt-4 rounded-xl border border-amber-300/15 bg-amber-300/[0.05] p-3 text-[11px] leading-relaxed text-amber-100">Do not extract or copy audio from Instagram Reels, films, commercial songs, or other protected sources unless you hold the required rights. Imported files stay in this browser session and are not uploaded to the database.</div><div className="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button onClick={onCancel} className="revrse-button-secondary">Cancel</button><button onClick={onConfirm} className="revrse-button-primary"><Check className="h-4 w-4" /> I confirm my rights</button></div></div></div>;
}
...
function Dashboard({ recents, serverProjects, sharedTemplates, favoriteTemplateIds, accountReady, serverLoading, serverError, onNew, onPreset, onOpenCurrent, onOpenRecent, onOpenServer, onDuplicate, onRename, onDemo, onStudio, onOpenSharedTemplate, onToggleFavorite, onTemplate, onImport, onDelete }: { recents: RecentProject[]; serverProjects: ServerProjectSummary[]; sharedTemplates: SharedTemplateResource[]; favoriteTemplateIds: number[]; accountReady: boolean; serverLoading: boolean; serverError: boolean; onNew: (ratio: CanvasRatio) => void; onPreset: (preset: typeof canvasPresets[number]) => void; onOpenCurrent: () => void; onOpenRecent: (item: RecentProject) => void; onOpenServer: (id: number) => void; onDuplicate: (item: RecentProject) => void; onRename: (item: RecentProject) => void; onDemo: () => void; onStudio: () => void; onOpenSharedTemplate: (id: number, title: string) => void; onToggleFavorite: (templateId: number) => void; onTemplate: (template: typeof templateRecipes[number]) => void; onImport: () => void; onDelete: (id: string) => void }) {
  ...
  return <main className="mx-auto min-h-screen max-w-[1500px] px-4 py-5 sm:px-7 sm:py-8">
    ...
    <section className="grid gap-6 py-7 lg:grid-cols-[minmax(0,1fr)_20rem]">
      ...
      <aside className="rounded-xl border border-white/[0.08] bg-[#10131a] p-4"><p className="text-xs font-bold uppercase tracking-[0.14em] text-slate-500">Workspace principles</p><div className="mt-4 space-y-4 text-xs leading-relaxed text-slate-400"><p><strong className="font-medium text-slate-200">Your edit, your media.</strong> Import video, images, and audio you own or are allowed to use.</p><p><strong className="font-medium text-slate-200">No forced watermark.</strong> The project format and on-device editing tools add none.</p><p><strong className="font-medium text-slate-200">No hidden upload.</strong> Imports stay in the current browser session unless you explicitly save a project backup.</p></div><button onClick={onDemo} className="revrse-button-secondary mt-5 w-full"><Sparkles className="h-4 w-4" /> Open starter timeline</button></aside>
    </section>
  </main>;
}

function WorkspaceSidebar({ activePanel, project, onPanel }: { activePanel: Panel; project: EditorProject; onPanel: (panel: Panel) => void }) {
  const status = getLocalWorkspaceStatus(project);
  return <aside className="flex w-14 shrink-0 flex-col border-r border-white/[0.08] bg-[#0b0e14] py-2" aria-label="Workspace sidebar"><div className="flex-1 space-y-1 overflow-y-auto px-1.5">{panelItems.map(item => { const Icon = item.icon; const active = isWorkspacePanelActive(activePanel, item.id); return <button key={item.id} onClick={() => onPanel(item.id)} className={`group relative grid h-10 w-full place-items-center rounded-lg transition ${active ? "bg-cyan-300/12 text-cyan-100 shadow-[inset_0_0_0_1px_rgba(165,243,252,.14)]" : "text-slate-500 hover:bg-white/[0.055] hover:text-slate-200"}`} aria-label={item.label} title={item.label}><Icon className="h-4 w-4" /><span className="pointer-events-none absolute left-[calc(100%+0.55rem)] z-50 hidden whitespace-nowrap rounded-md border border-white/[0.1] bg-[#171c26] px-2 py-1 text-[10px] font-medium text-slate-100 shadow-xl group-hover:block">{item.label}</span></button>; })}</div><div className="mx-1.5 mt-2 border-t border-white/[0.08] pt-2"><div className="grid place-items-center rounded-lg bg-emerald-300/[0.055] px-1 py-2 text-center" title={status.ariaLabel}><Check className="h-3.5 w-3.5 text-emerald-200" /><span className="mt-1 text-[8px] font-semibold uppercase tracking-[0.08em] text-emerald-100">{status.label}</span><span className="mt-0.5 text-[8px] text-slate-500">{status.detail}</span></div></div></aside>;
}

function EditorWorkspace(props: {
  project: EditorProject; assets: EditorAsset[]; selectedClip: TimelineClip | null; selectedAsset?: EditorAsset; previewAsset?: EditorAsset; activeVisualClip?: TimelineClip | null; activeTextClips: TimelineClip[]; currentTime: number; duration: number; playing: boolean; voiceRecording: boolean; activePanel: Panel; mobilePanelOpen: boolean; search: string; assetView: "grid" | "list"; showGrid: boolean; showSafeZones: boolean; snapping: boolean; loopPlayback: boolean; previewScale: number; timelineZoom: number; tool: Tool; draggingId: string | null; dropHighlight: boolean; ruler: number[]; pxPerSecond: number; historyAvailable: boolean; futureAvailable: boolean; keyframeProperty: TimelineClip["keyframes"][number]["property"]; captionDraft: string; assistantCommand: string;
  ...
  return <div className="h-[100dvh] min-h-[42rem] overflow-hidden bg-[#0a0d12]">
    <header className="flex h-14 items-center justify-between border-b border-white/[0.08] bg-[#10131a] px-2 sm:px-4">
      ...
      <div className="flex items-center gap-1"><button onClick={props.onCommand} className="revrse-icon-button" aria-label="Open command palette"><CommandIcon className="h-4 w-4" /></button><button onClick={props.onShortcuts} className="revrse-icon-button hidden sm:grid" aria-label="Keyboard shortcuts"><Keyboard className="h-4 w-4" /></button><button onClick={props.onSettings} className="revrse-icon-button hidden sm:grid" aria-label="Project settings"><Settings2 className="h-4 w-4" /></button><button onClick={props.onAccountSave} disabled={props.syncing} className="revrse-button-secondary hidden lg:inline-flex disabled:cursor-wait disabled:opacity-60"><Save className="h-4 w-4" /> {props.syncing ? "Saving" : "Save copy"}</button><button onClick={props.onImport} className="revrse-button-secondary hidden md:inline-flex"><Upload className="h-4 w-4" /> Import</button>{props.voiceRecording ? <><button onClick={() => props.onStopVoiceOver()} className="revrse-button-secondary hidden md:inline-flex border-rose-300/30 text-rose-100"><Mic className="h-4 w-4 animate-pulse" /> Stop voice</button><button onClick={() => props.onStopVoiceOver(true)} className="revrse-icon-button text-rose-200" aria-label="Discard voice-over"><X className="h-4 w-4" /></button></> : <button onClick={props.onStartVoiceOver} className="revrse-button-secondary hidden md:inline-flex" aria-label="Record a local voice-over"><Mic className="h-4 w-4" /> Voice-over</button>}<button onClick={props.onExport} className="revrse-button-primary"><Download className="h-4 w-4" /><span className="hidden sm:inline">Export</span></button></div>
    </header>
...
function ProviderMusicDiscovery() {
  const [query, setQuery] = useState("");
  const [duration, setDuration] = useState<"any" | "short" | "medium" | "long">("any");
  const [licenceLinkOnly, setLicenceLinkOnly] = useState(false);
  const providerStatus = trpc.musicDiscovery.status.useQuery(undefined, { retry: false });
  const provider = providerStatus.data as MusicProviderStatus | undefined;
  const providerSearch = trpc.musicDiscovery.search.useQuery(
    { query: query.trim() || "revrse" },
    { enabled: Boolean(provider?.configured && query.trim().length >= 2), retry: false },
  );
  const tracks = (providerSearch.data ?? []) as MusicProviderTrack[];
  const filteredTracks = tracks.filter(track => {
    const durationMatches = duration === "any"
      || (duration === "short" && track.durationSeconds <= 30)
      || (duration === "medium" && track.durationSeconds > 30 && track.durationSeconds <= 120)
      || (duration === "long" && track.durationSeconds > 120);
    return durationMatches && (!licenceLinkOnly || Boolean(track.licenseUrl));
  });

  return <details className="rounded-lg border border-emerald-300/15 bg-emerald-300/[0.035] p-3">
    <summary className="cursor-pointer text-xs font-semibold text-emerald-100">Optional royalty-free music discovery</summary>
    <div className="mt-3 space-y-2">
      <p className="text-[10px] leading-relaxed text-slate-400">Jamendo is used only for catalogue metadata when this deployment has its own app credential. REVRSE EDITOR does not stream, download, proxy, autoplay, or store Jamendo audio. Always open the provider page and verify the specific licence and intended use before obtaining audio independently.</p>
      {providerStatus.isLoading ? <p className="text-[10px] text-slate-500">Checking provider setup…</p> : !provider?.configured ? <div className="rounded-md border border-amber-300/15 bg-amber-300/[0.04] p-2.5 text-[10px] leading-relaxed text-slate-400"><p className="font-semibold text-amber-100">Discovery needs a Jamendo app credential.</p><p className="mt-1">This safe fallback intentionally returns no tracks until an owner adds a server-side client ID. Create a project in the official provider portal; never paste the ID into project files or client-side code.</p><a href="https://developer.jamendo.com/v3.0" target="_blank" rel="noreferrer" className="mt-2 inline-flex font-medium text-emerald-200 hover:text-emerald-100">Open Jamendo developer portal</a></div> : <><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search title, artist, or mood" className="revrse-search" /><div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]"><label><span className="sr-only">Filter discovery duration</span><select value={duration} onChange={event => setDuration(event.target.value as "any" | "short" | "medium" | "long")} className="h-8 w-full rounded-md border border-white/[0.1] bg-[#0c1017] px-2 text-[10px] text-slate-300 outline-none focus:border-emerald-300/60"><option value="any">Any duration</option><option value="short">Up to 30 seconds</option><option value="medium">30 seconds to 2 minutes</option><option value="long">Over 2 minutes</option></select></label><label className="flex min-h-8 items-center gap-2 rounded-md border border-white/[0.08] bg-black/10 px-2 text-[10px] text-slate-400"><input type="checkbox" checked={licenceLinkOnly} onChange={event => setLicenceLinkOnly(event.target.checked)} className="accent-emerald-300" /> Has licence link</label></div><p className="text-[10px] text-slate-500">Metadata-only results; no playback, download, or automatic import is available in this editor.</p>{providerSearch.isFetching ? <p className="text-[10px] text-slate-500">Searching provider catalogue…</p> : <div className="space-y-2">{filteredTracks.map(track => <article key={track.id} className="flex gap-2 rounded-md border border-white/[0.08] bg-white/[0.025] p-2.5">{track.artworkUrl ? <img src={track.artworkUrl} alt="" className="h-10 w-10 shrink-0 rounded object-cover" referrerPolicy="no-referrer" /> : <div className="grid h-10 w-10 shrink-0 place-items-center rounded bg-emerald-300/10 text-[10px] text-emerald-100">♫</div>}<div className="min-w-0 flex-1"><p className="truncate text-xs font-semibold text-slate-200">{track.title}</p><p className="mt-1 truncate text-[10px] text-slate-500">{track.artist} · {formatTime(track.durationSeconds)}</p><div className="mt-2 flex flex-wrap gap-x-3 gap-y-1"><a href={track.sourceUrl} target="_blank" rel="noreferrer" className="text-[10px] font-medium text-emerald-200 hover:text-emerald-100">Open provider page</a>{track.licenseUrl ? <a href={track.licenseUrl} target="_blank" rel="noreferrer" className="text-[10px] font-medium text-slate-300 hover:text-white">Check licence</a> : <span className="text-[10px] text-amber-100/80">Licence link unavailable — verify on provider page</span>}</div></div></article>)}{query.trim().length >= 2 && !filteredTracks.length && !providerSearch.isFetching && <p className="rounded-md border border-dashed border-white/[0.1] p-2 text-center text-[10px] text-slate-500">No provider metadata matched this search and selected filters.</p>}</div>}</>}</div>
  </details>;
}
...
function SoundLibraryPanelV2({ sounds, favoriteIds = [], accountReady, onUse, onFavorite, onRate, onPublish }: { sounds: SharedSoundResource[]; favoriteIds?: number[]; accountReady: boolean; onUse?: (sound: SharedSoundResource) => void; onFavorite?: (soundId: number) => void; onRate?: (resource: CommunityResource) => void; onPublish?: (file: File, input: SoundPublicationInput) => Promise<void> }) {
  ...
  const [licenseType, setLicenseType] = useState<"creator-owned" | "public-domain" | "royalty-free" | "permission">("creator-owned");
  const [creditLine, setCreditLine] = useState("");
  const [sourceUrl, setSourceUrl] = useState("");
  const [attested, setAttested] = useState(false);
  const categories = ["All", ...Array.from(new Set(sounds.map(sound => sound.category).filter(Boolean))).sort((a, b) => a.localeCompare(b))];
  const visible = sounds.filter(sound => {
    const haystack = `${sound.title} ${sound.description} ${sound.category} ${sound.moods} ${sound.licenseType} ${sound.creatorName ?? ""}`.toLowerCase();
    const durationMatches = durationFilter === "all" || (durationFilter === "short" && sound.durationMs <= 30_000) || (durationFilter === "medium" && sound.durationMs > 30_000 && sound.durationMs <= 120_000) || (durationFilter === "long" && sound.durationMs > 120_000);
    return haystack.includes(query.toLowerCase()) && (categoryFilter === "All" || sound.category === categoryFilter) && durationMatches;
  });
  const submit = async () => {
    if (!file || !title.trim() || description.trim().length < 10 || !category.trim() || !attested || !onPublish) return;
    await onPublish(file, { title: title.trim(), description: description.trim(), category: category.trim(), moods: moods.trim(), licenseType, creditLine: creditLine.trim(), sourceUrl: sourceUrl.trim() });
    setFile(null); setTitle(""); setDescription(""); setCategory("Music"); setMoods(""); setLicenseType("creator-owned"); setCreditLine(""); setSourceUrl(""); setAttested(false);
  };
  return <div className="space-y-3"><div className="rounded-lg border border-fuchsia-300/15 bg-fuchsia-300/[0.04] p-3"><p className="flex items-center gap-2 text-xs font-semibold text-fuchsia-100"><Music2 className="h-4 w-4" /> Licensed Sound Library</p><p className="mt-1.5 text-[11px] leading-relaxed text-slate-400">A growing in-editor library for creator-owned, public-domain, royalty-free, and expressly licensed music. Tracks are published by real creators with source and credit details; no catalogue entries, reviews, or music are fabricated.</p><p className="mt-2 rounded-md border border-fuchsia-200/10 bg-black/15 px-2 py-1.5 text-[10px] leading-relaxed text-fuchsia-50/80"><strong className="font-semibold text-fuchsia-100">In-editor use only.</strong> Add a permitted track to this project; REVRSE EDITOR does not offer standalone audio downloads. Any future provider catalogue must keep audio within the permitted project and export workflow.</p></div><ProviderMusicDiscovery /><div className="grid gap-2 sm:grid-cols-3"><div className="relative sm:col-span-3"><Search className="absolute left-3 top-2.5 h-3.5 w-3.5 text-slate-500" /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search title, creator, mood, genre, or licence" className="revrse-search" /></div><select value={categoryFilter} onChange={event => setCategoryFilter(event.target.value)} className="revrse-search pl-3 text-xs">{categories.map(item => <option key={item} value={item}>{item}</option>)}</select><select value={durationFilter} onChange={event => setDurationFilter(event.target.value)} className="revrse-search pl-3 text-xs"><option value="all">Any duration</option><option value="short">Up to 30 seconds</option><option value="medium">30 seconds to 2 minutes</option><option value="long">Over 2 minutes</option></select><p className="self-center text-[10px] text-slate-500">{visible.length} lawful {visible.length === 1 ? "track" : "tracks"}</p></div><div className="space-y-2">{visible.length ? visible.map(sound => <article key={sound.id} className="rounded-lg border border-white/[0.08] bg-white/[0.025] p-2.5"><div className="flex items-start gap-2"><span className="grid h-8 w-8 shrink-0 place-items-center rounded bg-fuchsia-300/10 text-fuchsia-100"><Music2 className="h-4 w-4" /></span><div className="min-w-0 flex-1"><p className="truncate text-xs font-semibold text-slate-200">{sound.title}</p><p className="mt-1 text-[10px] text-slate-500">{sound.category} · {formatTime(sound.durationMs / 1000)} · {sound.creatorName ?? "Creator"}</p><div className="mt-1 flex flex-wrap gap-1"><span className="rounded bg-fuchsia-300/10 px-1.5 py-0.5 text-[9px] text-fuchsia-100">{sound.licenseType.replace(/-/g, " ")}</span>{sound.moods && <span className="rounded bg-white/[0.05] px-1.5 py-0.5 text-[9px] text-slate-400">{sound.moods}</span>}</div><p className="mt-1 line-clamp-2 text-[10px] leading-relaxed text-slate-400">{sound.description}</p>{sound.creditLine && <p className="mt-1 text-[10px] text-slate-500">Credit: {sound.creditLine}</p>}{sound.sourceUrl && <a href={sound.sourceUrl} target="_blank" rel="noreferrer" className="mt-1 inline-flex text-[10px] font-medium text-emerald-200 hover:text-emerald-100">View stated source & licence</a>}</div></div><div className="mt-2 flex flex-wrap gap-1.5"><button onClick={() => onUse?.(sound)} className="revrse-mini-button">Use in edit</button><button onClick={() => onFavorite?.(sound.id)} className={`revrse-mini-button ${favoriteIds.includes(sound.id) ? "border-fuchsia-300/40 bg-fuchsia-300/10 text-fuchsia-100" : ""}`} aria-pressed={favoriteIds.includes(sound.id)}><Star className={`h-3 w-3 ${favoriteIds.includes(sound.id) ? "fill-current" : ""}`} /> {favoriteIds.includes(sound.id) ? "Saved" : "Save"}</button><button onClick={() => onRate?.({ type: "sound", id: sound.id, title: sound.title })} className="revrse-mini-button">Rate</button></div></article>) : <p className="rounded-lg border border-dashed border-white/[0.1] p-3 text-center text-[10px] leading-relaxed text-slate-500">No shared lawful sounds match these filters. Publish an original or properly licensed track to build this community library.</p>}</div><details className="rounded-lg border border-white/[0.08] bg-white/[0.02] p-3"><summary className="cursor-pointer text-xs font-semibold text-slate-200">Publish a lawful sound</summary><div className="mt-3 space-y-2"><p className="text-[10px] leading-relaxed text-slate-500">Upload MP3, M4A, WAV, OGG, or WebM audio up to 12 MB. Do not upload commercial music, film audio, copied memes, or any track you cannot lawfully redistribute.</p><input type="file" accept="audio/mpeg,audio/mp4,audio/wav,audio/ogg,audio/webm,.mp3,.m4a,.wav,.ogg,.webm" onChange={event => setFile(event.target.files?.[0] ?? null)} className="block w-full text-[10px] text-slate-400 file:mr-2 file:rounded file:border-0 file:bg-white/[0.08] file:px-2 file:py-1 file:text-[10px] file:text-slate-200" /><input value={title} onChange={event => setTitle(event.target.value)} placeholder="Sound title" className="revrse-search" /><div className="grid gap-2 sm:grid-cols-2"><input value={category} onChange={event => setCategory(event.target.value)} placeholder="Genre/category, e.g. Ambient" className="revrse-search" /><input value={moods} onChange={event => setMoods(event.target.value)} placeholder="Moods, e.g. calm, cinematic" className="revrse-search" /></div><select value={licenseType} onChange={event => setLicenseType(event.target.value as typeof licenseType)} className="revrse-search pl-3 text-xs"><option value="creator-owned">I own this original recording</option><option value="public-domain">Public-domain recording</option><option value="royalty-free">Royalty-free redistribution licence</option><option value="permission">Written distribution permission</option></select><input value={creditLine} onChange={event => setCreditLine(event.target.value)} placeholder="Required credit or attribution (if applicable)" className="revrse-search" /><input value={sourceUrl} onChange={event => setSourceUrl(event.target.value)} placeholder="https:// source or licence page (if applicable)" inputMode="url" className="revrse-search" /><textarea value={description} onChange={event => setDescription(event.target.value)} rows={3} placeholder="Rights and usage description (at least 10 characters)" className="revrse-search resize-none pl-3" /><label className="flex items-start gap-2 text-[10px] leading-relaxed text-slate-400"><input type="checkbox" checked={attested} onChange={event => setAttested(event.target.checked)} className="mt-0.5" /> I attest that I may distribute this audio in REVRSE EDITOR and that the stated source, licence, and required credits are accurate.</label><button onClick={() => void submit()} disabled={!accountReady || !file || !title.trim() || description.trim().length < 10 || !attested || !onPublish} className="revrse-button-primary w-full disabled:cursor-not-allowed disabled:opacity-50"><Upload className="h-3.5 w-3.5" /> Publish lawful sound</button>{!accountReady && <p className="text-[10px] text-slate-500">Sign in to publish. You can still browse and add visible lawful sounds to an edit.</p>}</div></details></div>;
}
...
function ExportDialog({ project, onClose, onProject, onSrt }: { project: EditorProject; onClose: () => void; onProject: () => void; onSrt: () => void }) {
  const [progress, setProgress] = useState(0);
  const [preparing, setPreparing] = useState(false);
  const [prepared, setPrepared] = useState(false);
  const timerRef = useRef<number | null>(null);
  const recordingCapabilities = getRecordingCapabilities(typeof MediaRecorder === "undefined" ? undefined : MediaRecorder.isTypeSupported.bind(MediaRecorder));
  const supportsWebm = recordingCapabilities.webm;
  const supportsMp4 = recordingCapabilities.mp4;
  ...
  useEffect(() => () => { clearPreparationTimer(); }, []);
  const cancelPreparation = () => { clearPreparationTimer(); setPreparing(false); setProgress(0); setPrepared(false); };
  const prepare = () => {
    clearPreparationTimer();
    setPreparing(true); setPrepared(false); setProgress(8);
    timerRef.current = window.setInterval(() => {
      setProgress(current => {
        const next = nextExportPreparationProgress(current);
        if (next === 100) { clearPreparationTimer(); setPreparing(false); setPrepared(true); }
        return next;
      });
    }, 120);
  };
  return <div className="revrse-dialog-backdrop"><div className="revrse-dialog"><button onClick={onClose} className="absolute right-4 top-4 text-slate-500 hover:text-slate-100" aria-label="Close export dialog"><X className="h-4 w-4" /></button><div className="grid h-10 w-10 place-items-center rounded-xl bg-cyan-300/10 text-cyan-100"><Download className="h-5 w-5" /></div><h2 className="mt-4 text-lg font-semibold text-slate-100">Export preparation</h2><p className="mt-2 text-sm leading-relaxed text-slate-400">Prepare an editable delivery package in this browser. It never starts playback or claims to encode a final video file when a real compositing encoder is unavailable.</p><div className="mt-4 space-y-2 rounded-lg border border-white/[0.08] bg-white/[0.025] p-3 text-xs"><div className="flex justify-between gap-3 text-slate-300"><span>Project canvas</span><span className="text-right">{project.canvas.width} × {project.canvas.height} · {project.canvas.fps} fps</span></div><div className="flex justify-between gap-3 text-slate-300"><span>Editable outputs</span><span className="text-right text-emerald-200">Project JSON · Caption SRT</span></div><div className="flex justify-between gap-3 text-slate-300"><span>Browser recording capability</span><span className="text-right">WebM {supportsWebm ? "available" : "not reported"} · MP4 {supportsMp4 ? "available" : "not reported"}</span></div></div><div className="mt-3 rounded-lg border border-amber-300/15 bg-amber-300/[0.04] p-3 text-[11px] leading-relaxed text-amber-50/75"><p className="font-semibold text-amber-100">Final-video limitation</p><p className="mt-1">Capability detection does not create an MP4 or WebM render. Final video encoding requires a supported compositor/encoder and can be demanding at 4K. Use the editable downloads now; add a verified encoder before promising a finished video export.</p></div><div className="mt-4"><div className="flex items-center justify-between text-[11px] text-slate-400"><span>{preparing ? "Preparing browser-local delivery package…" : prepared ? "Preparation complete — outputs are ready." : "Check browser support before downloading."}</span><span>{progress}%</span></div><div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-white/[0.08]"><div className="h-full bg-cyan-300 transition-[width] duration-200" style={{ width: `${progress}%` }} /></div><div className="mt-3 flex gap-2"><button onClick={prepare} disabled={preparing} className="revrse-button-primary flex-1 disabled:cursor-not-allowed disabled:opacity-50">{prepared ? <Check className="h-4 w-4" /> : <Download className="h-4 w-4" />}{prepared ? "Prepared" : "Prepare outputs"}</button>{(preparing || prepared) && <button onClick={cancelPreparation} className="revrse-button-secondary">Cancel</button>}</div></div><div className="mt-5 grid gap-2 sm:grid-cols-2"><button onClick={onProject} className="revrse-button-primary"><Download className="h-4 w-4" /> Project JSON</button><button onClick={onSrt} className="revrse-button-secondary"><FileText className="h-4 w-4" /> Caption SRT</button></div></div></div>;
}

function ReportResourceDialog({ resource, accountReady, pending, onClose, onSubmit }: { resource: CommunityResource; accountReady: boolean; pending: boolean; onClose: () => void; onSubmit: (input: { reason: "rights" | "copyright" | "harassment" | "spam" | "other"; details: string }) => void }) {
  const reasons = ["rights", "copyright", "harassment", "spam", "other"] as const;
  const [reason, setReason] = useState<typeof reasons[number]>("rights");
  const [details, setDetails] = useState("");
  return <div className="revrse-dialog-backdrop"><div className="revrse-dialog"><button onClick={onClose} className="absolute right-4 top-4 text-slate-500 hover:text-slate-100" aria-label="Close report form"><X className="h-4 w-4" /></button><div className="grid h-10 w-10 place-items-center rounded-xl bg-rose-300/10 text-rose-100"><Flag className="h-5 w-5" /></div><h2 className="mt-4 text-lg font-semibold text-slate-100">Report shared resource</h2><p className="mt-2 text-sm leading-relaxed text-slate-400">Report <span className="font-medium text-slate-200">{resource.title}</span> only for a genuine rights, safety, or quality concern. Reports are private and reviewed by the owner.</p><label className="mt-4 block text-xs font-medium text-slate-300">Reason<select value={reason} onChange={event => setReason(event.target.value as typeof reason)} className="revrse-search mt-1.5 w-full pl-3"><option value="rights">Licence or ownership concern</option><option value="copyright">Copyright concern</option><option value="harassment">Harassment or harmful content</option><option value="spam">Spam or misleading resource</option><option value="other">Other concern</option></select></label><label className="mt-3 block text-xs font-medium text-slate-300">Details<textarea value={details} onChange={event => setDetails(event.target.value.slice(0, 600))} rows={4} maxLength={600} className="revrse-search mt-1.5 w-full resize-none pl-3" placeholder="Explain the concern (optional, up to 600 characters)" /></label>{!accountReady && <p className="mt-3 text-xs text-amber-100">Sign in before submitting a report.</p>}<div className="mt-4 flex gap-2"><button onClick={onClose} className="revrse-button-secondary flex-1">Cancel</button><button onClick={() => onSubmit({ reason, details: details.trim() })} disabled={!accountReady || pending} className="revrse-button-primary flex-1 disabled:cursor-not-allowed disabled:opacity-50"><Flag className="h-4 w-4" />{pending ? "Sending…" : "Send report"}</button></div></div></div>;
}

function ModerationDialog({ reports, pending, onClose, onResolve }: { reports: Array<{ id: number; reporterName: string | null; resourceType: string; resourceId: number; reason: string; details: string; createdAt: Date }>; pending: boolean; onClose: () => void; onResolve: (id: number, status: "resolved" | "dismissed", moderatorNote: string) => void }) {
  const [notes, setNotes] = useState<Record<number, string>>({});
  return <div className="revrse-dialog-backdrop"><div className="revrse-dialog max-h-[88dvh] overflow-y-auto"><button onClick={onClose} className="absolute right-4 top-4 text-slate-500 hover:text-slate-100" aria-label="Close moderation queue"><X className="h-4 w-4" /></button><div className="grid h-10 w-10 place-items-center rounded-xl bg-amber-300/10 text-amber-100"><ShieldCheck className="h-5 w-5" /></div><h2 className="mt-4 text-lg font-semibold text-slate-100">Owner moderation queue</h2><p className="mt-2 text-sm leading-relaxed text-slate-400">Open reports are visible only to the project owner. Resolving a report records a moderation outcome; it does not automatically remove media.</p><div className="mt-4 space-y-3">{reports.length ? reports.map(report => <article key={report.id} className="rounded-xl border border-white/[0.08] bg-white/[0.025] p-3"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold text-slate-100">{report.resourceType} #{report.resourceId} · {report.reason}</p><p className="mt-1 text-[10px] text-slate-500">From {report.reporterName ?? "Creator"} · {new Date(report.createdAt).toLocaleString()}</p></div><Flag className="h-4 w-4 shrink-0 text-rose-200" /></div>{report.details && <p className="mt-2 text-xs leading-relaxed text-slate-300">{report.details}</p>}<textarea value={notes[report.id] ?? ""} onChange={event => setNotes(current => ({ ...current, [report.id]: event.target.value.slice(0, 500) }))} rows={2} className="revrse-search mt-3 w-full resize-none pl-3" placeholder="Owner note (optional)" /><div className="mt-2 flex gap-2"><button onClick={() => onResolve(report.id, "resolved", notes[report.id] ?? "")} disabled={pending} className="revrse-button-primary flex-1 disabled:opacity-50">Resolve</button><button onClick={() => onResolve(report.id, "dismissed", notes[report.id] ?? "")} disabled={pending} className="revrse-button-secondary flex-1 disabled:opacity-50">Dismiss</button></div></article>) : <div className="rounded-xl border border-dashed border-white/[0.12] p-5 text-center text-sm text-slate-500">No open reports. REVRSE EDITOR does not create sample moderation records.</div>}</div></div></div>;
}
