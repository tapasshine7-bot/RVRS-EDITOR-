  ...
  return <main className="mx-auto min-h-screen max-w-[1500px] px-4 py-5 sm:px-7 sm:py-8">
    ...
    <section className="grid gap-6 py-7 lg:grid-cols-[minmax(0,1fr)_20rem]">
      <div>
        ...
        <div className="mt-7 flex items-center justify-between"><div><p className="text-sm font-semibold text-slate-100">Recent projects</p><p className="mt-1 text-xs text-slate-500">Project metadata stays in this browser. Re-import files after reopening a browser-local project.</p></div><button onClick={onOpenCurrent} className="revrse-mini-button"><FolderOpen className="h-3.5 w-3.5" /> Open current</button></div>
        <div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{recents.length ? recents.map(item => <article key={item.id} className="overflow-hidden rounded-xl border border-white/[0.08] bg-[#11141b]"><div className="relative h-24 bg-[radial-gradient(circle_at_25%_20%,rgba(34,211,238,.2),transparent_30%),linear-gradient(120deg,#111827,#080b12)]"><div className="absolute inset-4 flex items-end justify-between"><span className="rounded bg-black/25 px-2 py-1 text-[10px] font-semibold text-cyan-100">{item.aspectRatio}</span><span className="font-mono text-[10px] text-slate-400">{formatTime(item.duration)}</span></div></div><div className="p-3"><p className="truncate text-sm font-semibold text-slate-100">{item.name}</p><p className="mt-1 text-[11px] text-slate-500">{item.canvas.width} × {item.canvas.height} · edited {formatDate(item.updatedAt)} · created {formatDate(item.createdAt)}</p><div className="mt-3 flex flex-wrap gap-1.5"><button onClick={() => onOpenRecent(item)} className="revrse-mini-button bg-white/[0.045] text-slate-200">Open</button><button onClick={() => onDuplicate(item)} className="revrse-mini-button" aria-label={`Duplicate ${item.name}`}><Copy className="h-3.5 w-3.5" /></button><button onClick={() => onRename(item)} className="revrse-mini-button" aria-label={`Rename ${item.name}`}><FileText className="h-3.5 w-3.5" /></button><button onClick={() => onDelete(item.id)} className="revrse-mini-button text-slate-500 hover:text-rose-200" aria-label={`Delete ${item.name}`}><Trash2 className="h-3.5 w-3.5" /></button></div></div></article>) : <div className="col-span-full rounded-xl border border-dashed border-white/[0.12] bg-white/[0.018] p-8 text-center"><FolderOpen className="mx-auto h-5 w-5 text-slate-600" /><p className="mt-3 text-sm text-slate-300">No browser-local projects yet</p><p className="mt-1 text-xs text-slate-500">Create a project or open the original editable demo.</p></div>}</div>
